...
export default function Home() {
  ...
  return (
    <div className="bg-[#FFF9F6] text-[#5c4f4f] font-['Pretendard','Noto Sans KR',sans-serif]">
      {/* Header */}
      <header className="fixed top-0 w-full bg-white shadow-md z-50">
        <div className="max-w-screen-xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className={`md:flex gap-6 text-sm font-medium ${menuOpen ? 'block mt-4' : 'hidden md:flex'}`}>
      ...
      {/* Hero */}
      <section className="pt-32 pb-20 text-center bg-gradient-to-br from-[#F7BFBF] to-[#FFF9F6]">
        <div className="max-w-screen-md mx-auto px-4">
          <h1 className="text-3xl md:text-5xl font-bold leading-tight mb-4">
            “이 사람이 맞을까?" <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d88] to-[#7fc8cc]">
      ...
      <section className="bg-[#F7BFBF] text-[#5c4f4f] py-16 text-center">
        ...
        <div className="flex justify-center gap-12 font-bold text-xl">
          <div>
            <div>{users.toLocaleString()}+</div>
            <p className="text-sm mt-1 text-[#84757a]">이용자 수</p>
          </div>
          <div>
            <div>{reports.toLocaleString()}+</div>
            <p className="text-sm mt-1 text-[#84757a]">감정 리포트 생성</p>
          </div>
          <div>
            <div>{simulations.toLocaleString()}+</div>
            <p className="text-sm mt-1 text-[#84757a]">시뮬레이션 진행</p>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-20">
        <div className="max-w-screen-lg mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <FeatureCard icon={<MessageCircle size={32} />} title="AI 저널링 챗봇" desc="마음속 이야기를 정리해주는 AI와 대화하며 감정을 객관적으로 돌아보세요." />
...
const pricingPlans = [
  {
    ...
    desc: '간단한 감정/불안 유형 테스트 (5~6문항) + 1문단 요약 피드백',
    price: '무료',
  },
  {
    stage: '2단계',
    title: '감정 리포트 미리보기',
    desc: '감정 진단 결과 요약 + 시뮬레이션 맛보기 + 리포트 프레임만 보여줌',
    price: '3,900원',
  },
  {
    stage: '3단계',
    title: '정식 감정 리포트 PDF',
    desc: '감정 요약 + 갈등 원인 분석 + AI 시뮬 대화 요약 + 대응 가이드 등 포함된 완성형 리포트',
    price: '14,900원',
  },
  {
    stage: '4단계',
    title: '감정 정리 챗봇 + 리포트 + 시뮬 전체',
    desc: '모든 기능 통합: 챗봇 정리 + 시뮬레이션 다회차 + 리포트 PDF 포함',
    price: '19,900원 (프리미엄 번들)',
  },
];